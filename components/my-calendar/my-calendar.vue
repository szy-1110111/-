<template>
  ...
  <view class="Cbody">
    <!-- 日历头部年月显示 -->
    <view class="Cheader"
      >{{ nowYear || "--" }} 年 {{ nowMonth || "--" }} 月</view
    >
    <!-- 日历星期 -->
    <view class="Cweek">
      <view class="weekText" v-for="(item, index) in weeksTxt" :key="index">{{
        item
      }}</view>
    </view>
    <swiper class="Cmain" :current="current" circular @change="changeSwiper">
      <!-- 轮播页码 -->
      <swiper-item
        v-for="(currentData, index) in swiperPageM_data"
        :key="index"
      >
        <view class="dateTable">
          ...
          >
            ...
            <view
              ...
              :style="{
                background: `linear-gradient(to bottom, var(${item.patchStatus[0]}) 55%, var(${item.patchStatus[1]}) 50%)`,
              }"
              class="mark"
...
const init = () => {
  ...
  swiperPageM_data.value.forEach((_, i) => {
    const { year, month } =
      i === 1
        ? { year: nowYear.value, month: nowMonth.value }
        : getOperateMonthDate(nowYear.value, nowMonth.value, i - 1);
...
const calculateGrids = (year: number, month: number, current: number): void => {
  ...
  swiperPageM_data.value[current].splice(
    0,
    swiperPageM_data.value[current].length,
    ...newCalendarDays
  );
};
...
const fullDate = (year: number, month: number, current: number): void => {
  ...
  for (let i = 0; i < firstDayOfWeek; i++) {
    ...
    swiperPageM_data.value[current][i].fullDate =
      lastMonth.year + "-" + lastMonth.month + "-" + date;
  }
  // 日历后补下个月初日期
...
const selectData = (data: string) => {
  ...
  swiperPageM_data.value[current.value].map((item: any, index: number) => {
    item.isSelect = false;
    item.fullDate == data
      ? (swiperPageM_data.value[current.value][index].isSelect = true)
      : null;
    return item;
  });
};
